...
const searchEventById = async(req,res)=>{
    const id = req.params.id
    try{
        const sEvent = await Event.findById(id)
        if(!sEvent){
            return res.status(404).send({message:'no se encontro el evento'});
        }
        return res.status(200).json(sEvent)
    }catch(e){
        throw new Error(e)
    }
}
